...
class Matrix:
    ...
    def __init__(self, B, angles, case):
        self.B = B
        self.angles = angles
...
class MatrixWigner(Matrix):
    ...
    def param(self,ii):
        ...
        norm_gamma = np.sqrt((math.factorial(self.s_plus)*math.factorial(self.s_plus + self.mu_plus + self.vu_plus))/
                             (math.factorial(self.s_plus + self.mu_plus)*(math.factorial(self.s_plus + self.vu_plus))))
    ...
    def deriv_wigner(self, theta, wignerd, normalize):
        ...
        Jacobi_last = (Plkn(self.s_plus - 1, self.mu_plus + 1, self.vu_plus + 1, np.cos(theta)))/normalize
        
        ## Derivative
        
        wignerd_deriv =(((self.mu_plus*np.sin(theta))/(2.0*(1 - np.cos(theta))) -
                         (self.vu_plus*np.sin(theta))/(2.0*(1 + np.cos(theta))))*wignerd  -
                        (np.sin(theta)*self.weight*(self.mu_plus + self.vu_plus + self.s_plus + 1)*0.5*
                         (np.sin(theta/2.0)**self.mu_plus)*(np.cos(theta/2.0)**self.vu_plus)*
                         (Jacobi_last)))
    ...
    def gen_matrix(self):
        ...
        for ii in range(self.N):
            ...
            A[:,ii] = (np.exp(-1j*self.deg_order[ii,1]*phi)*self.wigner_d( theta)*
...
class MatrixSNF(Matrix):    
    ...
    def gen_matrix(self):
        ...
        dmm_plus1 = np.zeros((self.m, self.N//2), dtype = np.float64)
        dmm_plus2 = np.zeros_like(dmm_plus1)
        
        dmm_min1  = np.zeros((self.m, self.N//2), dtype = np.float64)
        ...
        d_dmm_plus1 = np.zeros((self.m, self.N//2), dtype = np.float64)
        d_dmm_plus2 = np.zeros_like(d_dmm_plus1)
        
        d_dmm_min1 = np.zeros((self.m, self.N//2), dtype = np.float64)
