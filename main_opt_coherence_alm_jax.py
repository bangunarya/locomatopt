...
from locomatopt.auglmethod_jax import run_algo
...
def coherence_optimization(
    ...
    for idx in range(len(m)):
        ...
        # Run algorithm
        res_ang, coh = run_algo(ang, num_iter, optimizer, step_size, deg_order)
       
                       
        alm_ang_sample_jax.append(res_ang)
        alm_coh_sample_jax.append(coh)
        
        
        
        print('Sample (m): ', m[idx], ' ALM ', 'Coherence', coh)
     
        
    total_result = {'m': m, 'B': jnp.max(deg_order[0]),
                    'alm_ang': alm_ang_sample_jax, 'alm_coh': alm_coh_sample_jax}
...
if __name__ == '__main__':
    ...
    filename = basis + '' + case + '' + 'update_all' + '_alm_jax'
